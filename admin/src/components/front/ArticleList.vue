<template>
    <div>
        <div class="articles" v-if=" list.length !== 0">
            <article v-for="item in list" :key="item.article_id" v-if="item.article_pass && (item.tags_name === $route.query.tags_name || $route.query.tags_name === undefined)">
        ...
        watch: {
            ...
            '$store.state.search': function (newVal, oldVal) {
                ...
                if (this.$route.query.tags_name === undefined) {
                    ...
                    this.list = this.articles.filter(e => e.article_title.indexOf(newVal) > -1 || e.article_author.indexOf(newVal) > -1 && e.tags_name === this.$route.query.tags_name).slice((this.page - 1) * this.size, (this.page - 1) * this.size + this.size)             
                }
